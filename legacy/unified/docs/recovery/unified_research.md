...
> Legacy workflow reference, retained on 2026-09-05. Commands below run from the
> repository root; reproduction now requires `--profile legacy`. Sample counts
> and results describe the old reference, not a verified current local dataset.
> See the [active overview](../../README.md) and [historical recovery guide](../archive/README.md).
> Phase 4 moved retired entry points into `legacy/unified/`. Paths and commands
> below describe the preserved pre-migration layout; use the
> [current recovery instructions](phase4_recovery.md) for current routing.
...
```bash
python scripts/reproduce.py --profile legacy  # see REPRODUCING.md for stages,
...
- Chain and IV caches live under `data/raw/options_screen/{ticker}/`. Preserve
  them when reviewing old results; acquiring replacement data is a separate step.
