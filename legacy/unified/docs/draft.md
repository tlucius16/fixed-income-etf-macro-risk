...
**Travon Lucius**<br>
travon.lucius@blackrock.com<br>
August 2026
...
This paper asks whether that fragility is visible in public return data, whether investors are compensated for bearing it, and whether the exchange listed-options market can transfer the resulting tail risk. We define fragility as trailing 12-week volatility of weekly excess returns. The measure is intentionally simple. It is not a structural estimate of liquidity, duration, or credit risk; rather, it is a transparent signal of recent instability in the ETF price itself. We validate the measure with downside volatility and drawdown variants, then compare the scale of observed fragility with the exchange-listed hedge capacity available to investors.
...
The universe is survivorship-biased as it is based on funds present in the ETFDB screener as of April 2026. This possibly understates fragility among the riskiest funds, since liquidated or merged ETFs are omitted. We therefore report a robustness check restricted to the 183 ETFs with full history from 2016.

### 2.2 Price Data and Excess Returns

Daily adjusted close prices are obtained from Yahoo Finance and aggregated to weekly Friday-close prices. Weekly simple returns are computed from consecutive Friday closes. The risk-free proxy is the FRED 3-month Treasury bill secondary-market rate (DTB3), quoted as an annualized percentage on a discount basis. We approximate the corresponding weekly decimal rate as \(r_{f,t}=(DTB3_t/100)/52\). Excess returns are defined as:
...
The analysis is reduced-form. The macro regressions describe co-movement, not causal transmission. The predictive regressions avoid mechanical look-ahead as fragility uses returns through $t$ and forward outcomes begin at $t+1$, but persistence in volatility means past and future risk are naturally related. The stress index uses full sample standardization in the baseline, which is appropriate for retrospective classification but not for real-time trading; an expanding-window version addresses this concern in Section 7. High-stress weeks cluster in the COVID-19 shock and the 2022 rate-hike cycle, so the stress evidence should be read as evidence from those episodes rather than from a broad sample of independent crises. Portfolio tests are gross of transaction costs, so the weekly tilt should be interpreted as a risk nmanagement screen rather than a trading strategy/signal.
...
Replacing the full sample stress index with an expanding-window version identifies 36 stress weeks, with 96.3% week-level agreement between the two classifications. The interaction remains negative (−0.290 versus −0.325 under full sample standardization), so the stress-regime findings are not an artifact of post-ex standardization.
