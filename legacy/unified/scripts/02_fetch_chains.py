...
``legacy/unified/scripts/03_concat_screen.py`` afterward to rebuild the canonical processed
...
-----
    ...
    python legacy/unified/scripts/02_fetch_chains.py

    # Fetch selected tickers only:
    python legacy/unified/scripts/02_fetch_chains.py --tickers TLT IEF LQD

    # Fetch selected snapshot dates only:
    python legacy/unified/scripts/02_fetch_chains.py --snap-dates 2025-01-02 2025-04-01
"""
...
from legacy.unified.src.data.options import run_screen
from legacy.unified.src.data.options_universe import UNIVERSE
...
def _fetch_chains(tickers: list[str], snap_dates: list[str]) -> pd.DataFrame:
    ...
    logger.info(
        "Cached %d fetched chain rows. Run legacy/unified/scripts/03_concat_screen.py to "
