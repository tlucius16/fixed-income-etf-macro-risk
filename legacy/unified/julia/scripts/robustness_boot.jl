...
#
# Usage:
#   julia --project=legacy/unified/julia legacy/unified/julia/scripts/robustness_boot.jl
#   julia --project=legacy/unified/julia legacy/unified/julia/scripts/robustness_boot.jl --reps 999 --seed 20260709
...
const LEGACY = normpath(joinpath(@__DIR__, "..", ".."))
const REPO = normpath(joinpath(LEGACY, "..", ".."))
...
const REFERENCE_ROBUSTNESS = joinpath(LEGACY, "docs", "hedge_capacity", "tables", "robustness_spec0.csv")
const DEFAULT_OUT = joinpath(LEGACY, "docs", "hedge_capacity", "tables", "robustness_boot.csv")
...
function build_side_capacity()
    isfile(SIDE_CAPACITY) || error(
        "side_capacity.csv not found; run legacy/unified/scripts/07_robustness_ladder.py first."
    )
