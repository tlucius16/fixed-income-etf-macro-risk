...
Usage
-----
    from legacy.unified.src.data.options import fetch_atm_iv_30d, run_screen, concat_results
"""
...
from legacy.unified.src.data.options_universe import LIQUIDITY_SCORE_MIN

logger = logging.getLogger(__name__)

_IV_SEARCH_BOUNDS   = (1e-4, 20.0)
_MIN_OPTION_PRICE   = 0.01
_IV_CACHE_DIR       = config.OPTIONS_SCREEN_RAW_DIR
_SCREEN_CACHE_DIR   = config.OPTIONS_SCREEN_RAW_DIR
...
def _rf_map(start: str, end: str) -> dict[date, float]:
    ...
    if not api_key:
        ...
        return {}
    try:
        from legacy.unified.src.data.macro import fred_series, make_fred_session
...
def concat_results(
    cache_dir:        Path  = _SCREEN_CACHE_DIR,
    out_dir:          Path  = config.OPTIONS_SCREEN_DIR,
    max_rel_spread:   float = config.MAX_REL_SPREAD,
    dte_min:          int   = config.DTE_MIN,
    dte_max:          int   = config.DTE_MAX,
    delta_lo:         float = config.DELTA_LO,
